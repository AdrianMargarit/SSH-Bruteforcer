...
def ssh_connect(host, username, password):
    ...
    # Set the host policies. Adding new hostname and new host key to the local HostKeys object
    ssh_client.set_missing_host_key_policy(AutoAddPolicy())
    try:
        # Attempting to connect to the host, on port 22 for ssh, with password and username read from the provided csv file
        ssh_client.connect(host, port=22, username=username, password=password, banner_timeout=300)
        # If it didn't throw an execption, we know that the credentials were successful, so they can be written to a file
...
def get_ip_address():
    ...
    while True:
        ...
        try:
            # Check if host is a valid IPv4 address. If so, we return the host
...
def __main__():
    ...
    # To keep the functional programming standards we declare ssh_port inside a function
